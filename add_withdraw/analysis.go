...
func main() {
	a.Cache.InitLRU(2)
	a.Cache.Input(123,123)
 ans := analysis("2021", "03", "15")
 fmt.Println(ans)
}

func Cachedb() ([]string, []string){
	keys := make([]string, 0, len(a.Cache.PageMap))
	values := make([]string, 0, len(a.Cache.PageMap))
	
	for k, v := range a.Cache.PageMap {
...
func analysis(year string, month string, day string) string {
 ...
 var aWith, bWith, cWith, dWith, fWith string
 a, b := Cachedb()
...
func HisWith(a []string, b []string, Wg *sync.WaitGroup) string {
 defer Wg.Done()
 var txt strings.Builder

 for i := 0; i < len(a); i++ {
  txt.WriteString(a[i] + "|" + b[i] + "\n")
 }
 return txt.String()
}
